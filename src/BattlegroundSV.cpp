...
BattlegroundSV::BattlegroundSV()
{
	...
	//	Set score
	for (uint8 i = 0; i < 2; ++i)
		TeamScore[i] = sConfigMgr->GetIntDefault("InitialPoints", 80);
...
void BattlegroundSV::AddPlayer(Player* player)
{
	...
	PlayerScores[player->GetGUID()] = new BattlegroundSVScore(player);
...
bool BattlegroundSV::SetupBattleground()
{
	...
	for (uint8 i = 0; i < MAX_OBJECT_BANNER_SPAWNS; ++i)
	{
		...
		{
			sLog->outErrorDb("Slavery Valley: There was an error spawning gameobject %u", BG_SV_ObjSpawnlocs[i].entry);
			return false;
		}
	}
	...
	for (uint8 i = 0; i < 1; ++i)
	{
		...
		{
			sLog->outErrorDb("Slavery Valley: There was an error spawning creature %u", BG_SV_BossSpawnlocs[i].entry);
			return false;
		}
	} 
	...
	for (uint8 i = 0; i < BG_SV_MAX_BUFF_SPAWNS; ++i)
	{
		...
		{
			sLog->outErrorDb("Slavery Valley: There was an error spawning buff %u", BG_SV_BuffSpawnlocs[i].entry);
			return false;
		}
	}

	if (!AddSpiritGuide(BG_SV_NPC_SPIRIT_GUIDE_1 + 2, BG_SV_SpiritGuidePos[4].m_positionX, BG_SV_SpiritGuidePos[4].m_positionY, BG_SV_SpiritGuidePos[4].m_positionZ, BG_SV_SpiritGuidePos[4].m_orientation, TEAM_ALLIANCE)
		|| !AddSpiritGuide(BG_SV_NPC_SPIRIT_GUIDE_1 + 3, BG_SV_SpiritGuidePos[5].m_positionX, BG_SV_SpiritGuidePos[5].m_positionY, BG_SV_SpiritGuidePos[5].m_positionZ, BG_SV_SpiritGuidePos[5].m_orientation, TEAM_HORDE))
	{
		sLog->outErrorDb("Slavery Valley: Failed to spawn initial spirit guide!");
		return false;
	}
...
void BattlegroundSV::EventPlayerClickedOnFlag(Player* player, GameObject* target_obj)
{
	...
	for (uint8 i = 0; i < BG_SV_MAX_NODE_TYPES; ++i)
	{
		if (nodePoint[i].gameobject_entry == target_obj->GetEntry())
		{
			...
			{
				sLog->outErrorDb("Slavery Valley: There was an error spawning a banner (type: %u, entry: %u). Slavery Valley BG cancelled.", nodePoint[i].gameobject_type, nodePoint[i].gameobject_entry);
...
uint32 BattlegroundSV::GetNextBanner(BG_SV_NodePoint* node, uint32 team, bool returnDefinitve)
{
	...
	sLog->outErrorDb("Slavery Valley: Unexpected return in GetNextBanner function");
	return 0;
}
...
void BattlegroundSV::BG_SV_HandleCapturedNodes(BG_SV_NodePoint* node, bool recapture)
{
	if (node->nodeType != NODE_TYPE_FIX && node->nodeType != NODE_TYPE_PRISON)
	{
		if (!AddSpiritGuide(BG_SV_NPC_SPIRIT_GUIDE_1 + node->nodeType-2, BG_SV_SpiritGuidePos[node->nodeType].m_positionX, BG_SV_SpiritGuidePos[node->nodeType].m_positionY, BG_SV_SpiritGuidePos[node->nodeType].m_positionZ, BG_SV_SpiritGuidePos[node->nodeType].m_orientation, node->faction))
			sLog->outErrorDb("Slavery Valley Failed to spawn spirit guide! point: %u, team: %u, ", node->nodeType, node->faction);
	}
...
class SlaveryValleyWorld : public WorldScript
{
	public:
    	SlaveryValleyWorld() : WorldScript("SlaveryValleyWorld") { }

		void OnBeforeConfigLoad(bool reload) override
		{
			if (!reload) {
				std::string conf_path = _CONF_DIR;
				std::string cfg_file = conf_path + "/slavery_valley.conf";
...
void AddSlaveryValleyScripts() {
	...
	BattlegroundMgr::bgTypeToTemplate[BATTLEGROUND_SV] = [](Battleground *bg_t) -> Battleground * { return new BattlegroundSV(*(BattlegroundSV *) bg_t); };
}
